...
1. **`CORE-001`** Агент не додумывает отсутствующее продуктовое решение.
2. **`GIT-001`** Прямой коммит в `main` запрещён: только `feat/TASK-NNNN-*` и PR.
3. **`CHECK-001`** Воспроизводимая проверка сильнее мнения агента; обязательная проверка работает
   и при сбое запрещает продолжение.
4. **`CHANGE-001`** Реализация, тесты и затронутая документация изменяются вместе.
5. **`REVIEW-001`** Независимая проверка продуктового PR выполняется другим вызовом агента.
6. **`REVIEW-002`** Замечание сначала пытаются опровергнуть, затем исправляют.
7. **`RETRY-001`** Повторы ограничены; после исчерпания агент останавливается с диагностикой.
8. **`DONE-001`** `[x]` ставится только после слияния и успешной проверки в тестовой среде.
9. **`RISK-001`** Опасные или необратимые действия требуют подтверждения человека.
10. **`CHANGE-002`** Заглушка никогда не выдаётся за готовую реализацию.
